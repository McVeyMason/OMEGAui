...
IF "%choice%"=="%op%" (
	...
	echo.1> %file%\ProgramFiles\Temp\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Game. >> %file%\Users\%user%\logs\%now%.log
	echo.2> %file%\ProgramFiles\Temp\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Utility. >> %file%\Users\%user%\logs\%now%.log
	echo.3> %file%\ProgramFiles\Temp\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Other. >> %file%\Users\%user%\logs\%now%.log
	echo.4> %file%\ProgramFiles\Temp\success.temp
)
set /a op=%op%+1