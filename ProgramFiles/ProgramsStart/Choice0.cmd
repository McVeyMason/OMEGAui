...
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Browser. >> %file%\Users\%user%\logs\%now%.txt
	echo.1> %file%\ProgramFiles\ProgramsStart\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Game. >> %file%\Users\%user%\logs\%now%.txt
	echo.2> %file%\ProgramFiles\ProgramsStart\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Utility. >> %file%\Users\%user%\logs\%now%.txt
	echo.3> %file%\ProgramFiles\ProgramsStart\success.temp
)
set /a op=%op%+1
IF "%choice%"=="%op%" (
	echo.[%time%]:----Selected Other. >> %file%\Users\%user%\logs\%now%.txt
